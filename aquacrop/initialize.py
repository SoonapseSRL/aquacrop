...
__all__ = ['read_clock_paramaters', 'read_weather_inputs', 'read_canopy_inputs', 'read_model_parameters',
           'read_irrigation_management', 'read_field_management', 'read_groundwater_table', 'compute_variables',
           'compute_crop_calander', 'calculate_HIGC', 'calculate_HI_linear', 'read_model_initial_conditions',
           'create_soil_profile']
...
# Cell
# - CC UPDATE: ADDED
def read_canopy_inputs(ClockStruct, cc_df):
    """
    clip canopy cover to start and end simulation dates

    *Arguments:*\n

    `ClockStruct` : `ClockStructClass` : time paramaters

    `cc_df` : `pd.DataFrame` :  weather data

    *Returns:*

    `cc_df` : `pd.DataFrame`: clipped weather data

    """
    # TODO: return None or empty DataFrame
    if not isinstance(cc_df, pd.DataFrame) or cc_df.empty: return

    assert 'Date' in cc_df.columns
    ...
    # remove weather data outside of simulation dates
    cc_df = cc_df[cc_df.Date >= start_date]
    cc_df = cc_df[cc_df.Date <= end_date]

    # fill not present days with NaN
    # set Date as index
    cc_df = cc_df.set_index('Date')

    # create pandas data_range
    new_index = pd.date_range(start=start_date, end=end_date, freq='D')

    # reindex to new_index and fill with NaN
    cc_df = cc_df.reindex(new_index, fill_value=None)
